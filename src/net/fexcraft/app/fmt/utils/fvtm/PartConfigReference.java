...
public class PartConfigReference extends ConfigReference {
	...
	public PartConfigReference(){
		...
		entries.add(of("Installation", ENUM_SEPARATE).refs(PartInstallConfigReference.REFERENCES).def("default").add(of("Handler", STATIC)));
		VehicleConfigReference.addAttrsEntry(entries);
		entries.add(of("AttributeModifiers", OBJECT_KEY_VAL).add(TEXT_ENTRY));
		entries.add(of("Functions", OBJECT_KEY_VAL).add(of(ENUM_SEPARATE).refs(PartFunctionConfigReference.REFERENCES)));
