...
public class MRTRenderer extends ModelRendererTurbo.Renderer {
	...
	private static final float[] EMPTY = { 0, 0, 0, 0 }, FILLED = { 0, 0, 0, 1};
	...
	public void render(ModelRendererTurbo mrt, float scale){
        ...
        if(mrt.glId() == null || mrt.forcedRecompile){
            compileModel(obj, mrt, scale, mrt.linesColor != null);
        }
		...
		glUniform4fv(getUniform("line_color"), mrt.linesColor == null ? EMPTY : color(mrt.linesColor.toFloatArray()));
		//
	...
	private float[] color(float[] array){
		FILLED[0] = array[0];
		FILLED[1] = array[1];
		FILLED[2] = array[2];
		return FILLED;
	...
	private void compileModel(GlObj obj, ModelRendererTurbo mrt, float scale, boolean lines){
    	...
    	//obj.colors = new float[obj.size * 3];
    	obj.lights = new float[obj.size];
		//
		int ver = 0, uv = 0, nor = 0, col = 0, lig = 0;
    	for(int i = 0; i < mrt.getFaces().size(); i++){
    		...
    		for(int o = 0; o < order.length; o++){
    			...
    			/*if(lines){
    				byte[] rgb = mrt.linesColor.tofloatArray();
        			obj.colors[col++] = rgb[0];
        			obj.colors[col++] = rgb[1];
        			obj.colors[col++] = rgb[2];
    			}
    			else{
        			obj.colors[col++] = obj.colors[col++] = obj.colors[col++] = 0;
    			}*/
