...
public abstract class Editor extends Element {
	...
	public Editor(String id, String stylegroup){
		super(null, id, stylegroup, false);
		EDITORS.add(this);
		this.setColor(0xff999999);
		...
		button.getElements().add(new TextField(button, "mt", "multiplicator:field", 110, button.width - 144, 1){
			...
			public boolean processScrollWheel(int wheel){
				applyChange(FMTB.MODEL.multiply(wheel > 0 ? 2.0f : 0.5f));
				return true;
			}
		...
		button.getElements().add(new Icon(button, "mr", "multiplicator:icon", "icons/group_delete", 26, button.width - 30, 1){
			...
			public boolean processButtonClick(int x, int y, boolean left){
				((TextField)root.getElement("mt")).applyChange(1);
				FMTB.MODEL.rate = 1f;
				return true;
			}
		});
		this.setHoverColor(0xffffffff, false);
		this.repos();
	}
	...
	public Element repos(){
		x = 0;
		y = UserInterface.TOOLBAR.height + UserInterface.TOOLBAR.border_width;
		height = UserInterface.height - y;
		if(Settings.bottombar()) height -= 29;
		clearVertexes();
		this.reposContainers();
		return this;
	}
	...
	public void reposContainers(){
		...
		for(Container container : containers){
			container.y = y + pass;
			pass += container.getExpansionHeight() + 4;
			container.repos();
		}
	}
	...
	public static void show(String id){
		for(Editor edit : EDITORS)
			edit.setVisible(edit.id.equals(id));
	}
	...
	public static Editor get(String id){
		for(Editor edit : EDITORS)
			if(edit.id.equals(id)) return edit;
		return null;
	}
	...
	public static void toggle(String string, boolean close){
		Editor edit = get(string);
		if(close && edit != null && edit.isVisible()) hideAll();
		else show(string);
	}
	...
	public ArrayList<TextField> getFields(){
		...
		for(Element elm : elements)
			if(elm instanceof TextField) fields.add((TextField)elm);
		return fields;
	}
	...
	public static TextField getGlobalField(String string){
		for(TextField field : TextField.getAllFields())
			if(field.getId().equals(string)) return field;
		return null;
	}

	public TextField getLocalField(String string){
		Element elm = getElement(string);
		return elm instanceof TextField ? (TextField)elm : null;
	}
	...
	public static void toggleContainer(int i){
		Editor editor = getVisibleEditor();
		if(editor == null){
	...
	private static Editor getVisibleEditor(){
		for(Editor edit : EDITORS)
			if(edit.isVisible()) return edit;
		return null;
	}

	public static boolean anyVisible(){
		for(Editor edit : EDITORS)
			if(edit.isVisible()) return true;
		return false;
	}
