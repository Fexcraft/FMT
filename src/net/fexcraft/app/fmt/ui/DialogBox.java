...
public class DialogBox {
	...
	public static final Dialog show(String title, String text0, String text1, DialogTask but0, DialogTask but1, String... text){
        ...
        Dialog dialog = new Dialog(Translator.translate(title), 400, 70 + (text.length * 25)); dialog.setResizable(false);
        for(int i = 0; i < text.length; i++){
        	Label label = new Label(Translator.translate(text[i]), 10, 10 + (i * 25), 380, 20); dialog.getContainer().add(label);
        }
        if(text0 != null){
