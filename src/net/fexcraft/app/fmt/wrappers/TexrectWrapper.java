...
public class TexrectWrapper extends ShapeboxWrapper {
	
	public float[][] texcor = new float[6][0];
	
	public TexrectWrapper(GroupCompound compound){
		super(compound);
		for(int i = 0; i < 6; i++){
	...
	protected PolygonWrapper createClone(GroupCompound compound){
		TexrectWrapper wrapper = new TexrectWrapper(compound);
	...
	public ShapeType getType(){
		return ShapeType.TEXRECT;
	}
	
	@Override
	public float getFloat(String id, boolean x, boolean y, boolean z){
		if(id.startsWith("texpos")){ //Print.console(id, x);
	...
	public boolean setFloat(String id, boolean x, boolean y, boolean z, float value){
		...
		if(id.startsWith("texpos")){ //Print.console(id, x, value);
