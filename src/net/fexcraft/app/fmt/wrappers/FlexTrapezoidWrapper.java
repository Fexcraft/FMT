...
public class FlexTrapezoidWrapper extends BoxWrapper {
	...
	public FlexTrapezoidWrapper(GroupCompound compound){
		super(compound);
	}
	...
	public void recompile(){
		...
		turbo.addFlexTrapezoid(off.xCoord, off.yCoord, off.zCoord, size.xCoord, size.yCoord, size.zCoord, 1f,
			scales[0],scales[1],scales[2],scales[3],scales[4],scales[5], mr_side);
		...
		lines.addFlexTrapezoid(off.xCoord, off.yCoord, off.zCoord, size.xCoord, size.yCoord, size.zCoord, 0,
                scales[0], scales[1], scales[2], scales[3],scales[4],scales[5], mr_side); lines.lines = true;
	...
	public ShapeType getType(){
		return ShapeType.FLEXTRAPEZOID;
	}
	...
	public float getFloat(String id, boolean x, boolean y, boolean z){
		switch(id){
			...
			case "scale4":{return scales[4];}
			case "scale5":{return scales[5];}
	...
	public boolean setFloat(String id, boolean x, boolean y, boolean z, float value){
		...
		switch(id){
			...
			case "scale4":{scales[4] = value; return true;}
			case "scale5":{scales[5] = value; return true;}
	...
	protected JsonObject populateJson(JsonObject obj, boolean export){
		...
		if(scales[4] != 0) obj.addProperty("scale4", scales[3]);
		if(scales[5] != 0) obj.addProperty("scale5", scales[3]);
		if(mr_side != 0) obj.addProperty("side", mr_side);
		return obj;
	}


	public FlexTrapezoidWrapper setCoords(float var1, float var2, float var3, float var4, float var5, float var6, String side){
		scales = new float[]{var1,var2,var3,var4,5,6}; mr_side=getSide(side);
		return this;
	}
