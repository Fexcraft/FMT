...
public class TexrectWrapperA extends TexrectWrapperB {
	...
	public TexrectWrapperA(GroupCompound compound){
		...
		for(int i = 0; i < 6; i++){
			...
			texcor[i][0] = 0; texcor[i][1] = 0; texcor[i][2] = 0; texcor[i][3] = 0;
			texcor[i][4] = 0; texcor[i][5] = 0; texcor[i][6] = 0; texcor[i][7] = 0;
		}
	}
	...
	public float getFloat(String id, boolean x, boolean y, boolean z){
		if(id.startsWith("texpos") && id.contains(":")){
			String[] str = id.replace("texpos", "").split(":"); int i = Integer.parseInt(str[0]), j = Integer.parseInt(str[1]);
			return texcor[i][j];
	...
	protected float[][][] newTexturePosition(){
		...
		for(int i = 0; i < 6; i++){
			vecs[i] = new float[][]{
				...
				new float[]{ texcor[i][4], texcor[i][5] }
			};
		} return vecs;
	}
